# Pre-registration — OSF Preregistration template

**IDAlert Task 5.4, areal-unit messaging experiment**

Sections below map one-to-one onto the fields of the standard OSF
Preregistration form, in form order, so each can be pasted into the
corresponding box. Headings give the OSF field name.

This is the registration document of record. Technical background that does not
belong in a form field lives in `docs/operations/`, and the code that produces
...
*(add remaining IDAlert T5.4 contributors before submission)*
...
registration. The
advertising campaign has not launched. No comparison of any outcome between arms
...
effect falls to 0.72 (0.93 at 20%). The banded campaign structure is the
mitigation, and realized delivery spread will be reported whatever it shows. The campaign grouping described in *Study design* is the
mitigation, and realized delivery spread will be reported whatever it shows.
...
Reports in the second half of the post window run about 75% of the first half
(0.58–0.92 across 2021–2024), a mild further argument for concentrating
delivery early.
...
**Pre-submission regeneration.** The assignment and every figure above will be
regenerated against the completed 2026 pre-window data before this registration
is submitted, since the pre-window closed 14 August 2026 and enters the analysis
as a covariate. The eligibility rule, arm ratio, blocking variable, seed and
analysis plan are frozen as specified here and will not change; only the
resulting unit list and the calibrated power figures may.
...
described. Whether the framed arm runs one creative or both orientations is
listed under *Open items*; if both, the prevention-versus-promotion contrast is
exploratory and underpowered (see *Other planned analysis*).
...
participants, with 45% of municipality-seasons recording zero reporters), which
is why the design is powered on track participants. Power for the reporting
...
under **presence** attribution, in which a user is counted in every municipality
where they emitted a track. This is the attribution used in all data through
2025.
...
median post-window reporter count across 2021–2024). Nothing else changes.
...
municipalities — cannot invalidate it. That is not true of the parametric
alternatives, whose Type I error drifts upward with leakage (0.022 → 0.054 in
the leakage table above) precisely because leakage correlates neighbouring
outcomes.
...
exact). Both hypotheses sit within about 1.5 standard errors of 0.05 on this
check, and both were far worse on earlier, smaller candidate pools (H2 reached
0.22 without the cap and 0.10 without block terms); the large no-advertising
arm of this design is what tamed them. HC3 sits mildly below nominal
(0.02–0.03). A result whose significance depends on the gap between the two
...
alongside. With block dummies and a 42-unit no-ad arm, HC3 over-corrects and
runs conservative on the realized draw (H2 0.015–0.018, H1 0.035). Where a
block dummy isolates a single unit — possible in subset analyses — HC3 is
undefined and HC1 is substituted; the analysis code does this automatically.
...
track participants against 0.87–0.95 for report counts, with 43% of
...
4. Results generalize to Spanish municipalities of small to moderate existing
   Mosquito Alert activity that Google Ads can target by name — not to large
   cities, which the eligibility cap excludes, nor to municipalities with no
   prior activity.
...
2. **2026 is a genuinely low-participation season.** The pre-window total (304
   participants Spain-wide, against roughly 1,000–1,300 in comparable seasons)
   reflects reduced dissemination this year and apparently lower mosquito
   populations in much of Spain — it is not a data artefact. Consequences:
   (a) the 2026 pre-window covariate is weaker but valid, and should still be
   re-extracted with the October outcome pull to capture any late-uploading
   tracks; (b) baselines in the post window will likely run below the
   historical levels the power simulation resamples — which shrinks baseline
   noise while the install dose is unchanged, so stated power is conservative
   for H2 and approximately unchanged for H1, where delivery noise dominates;
...
4. ~~Calibration seasons~~ — decided 2026-08-14: extended to 2021–2025 and
   everything regenerated, including the reporting outcome (refreshed report
   export received the same day; reports now calibrate on 2021–2025 with
   windows matched exactly to the participants pipeline).
5. Fix the minimum-delivery threshold for secondary analysis 7.
6. Decide whether the framed arm runs one creative or both, with Google
   optimising between them, and deposit verbatim advertisement copy for all arms.
7. ~~CIREP~~ — approved as a modification to protocol 270 (originally
   approved 21 July 2022; modification approved 30 July 2026); recorded
